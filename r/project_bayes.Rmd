...
mcmc_obj_strong <- as.mcmc(posterior_samples_strong)
...
weak_coefs   <- colMeans(posterior_samples_weak)
strong_coefs <- colMeans(posterior_samples_strong)

cat("Weak model coefficients:\n")
print(weak_coefs)
cat("\nStrong model coefficients:\n")
print(strong_coefs)
```
...
# Actual default rate:
obs_default_rate <- mean(y)

# Predicted default rate (Weak):
pred_default_weak   <- mean(pred_probs_weak)
# Predicted default rate (Strong):
pred_default_strong <- mean(pred_probs_strong)
